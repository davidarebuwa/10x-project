...
describe("View List of users", () => {
  ...
  it("should be able to view the list of users", () => {
    ...
    cy.get(".container").within(() => {
      ...
      cy.get("[class='MuiGrid2-root MuiGrid2-grid-xs-4 css-wvws5z']").should(
        "have.length",
        6
      );
    });
  ...
  it("should be able to show each user details", () => {
    cy.get(".body").contains("Users");
    cy.get(":nth-child(1) > .MuiPaper-root > .MuiCardActionArea-root").click();
    cy.get(".item-detail-header > .MuiTypography-root").should(
      "have.text",
      "Profile"
    );
    //Verigy the delete and edit icons are visible
    cy.get(".item-detail-body").within(() => {
      cy.get("[data-testid='EditIcon']").should("be.visible");
      cy.get("[data-testid='DeleteIcon']").should("be.visible");
    });
    //Click on the first edit icon and verify that the edit user form is visible
  });
  ...
  it("should be able to edit a user", () => {
    ...
    cy.get("form > .MuiTypography-root").should(
      "have.text",
      "Create/Update User"
    );
    ...
    cy.get(".MuiAlert-message > div").should(
      "have.text",
      "User updated successfully"
    );
    cy.get(".MuiTypography-h6 > .MuiTypography-root").should(
      "have.text",
      "Jane@doe.com"
    );
  });
  ...
  it("should be able to delete a user", () => {
    //Delete the user
    cy.get(":nth-child(1) > .MuiPaper-root > .MuiCardActionArea-root").within(
      () => {
